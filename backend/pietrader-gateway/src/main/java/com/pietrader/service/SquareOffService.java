...
import com.pietrader.kafka.AlertProducer;
...
public class SquareOffService {
    ...
    public OrderResponse squareOff(String symbol, String reason) {
        ...
        if ("SUCCESS".equals(resp.getStatus()) || "PAPER".equals(resp.getStatus())) {
            ...
            alertProducer.warn("SQUAREOFF", symbol + " closed → " + reason + " pnl=₹" + String.format("%.2f", pnl));
        } else {
            alertProducer.critical("SQUAREOFF_FAILED", symbol + " exit FAILED: " + resp.getErrorMessage());
        }
        return resp;
    }
